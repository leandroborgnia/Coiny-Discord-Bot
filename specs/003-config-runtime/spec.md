...
**Input**: User description: "Configuration consolidation & containerized dev/prod runtime — consolidate all Spring configuration in application.yml (the project's existing format), eliminate any reliance on a .env file/dotenv for the app, supply secrets as environment variables injected by Docker Compose, and provide separate dev/prod launch scripts (Linux shell + PowerShell/WSL wrappers) that prompt for secrets and bring up the app and its database together. Maven is for building and the host test suite only — not a way to run the app."
...
- Q: Should the repo keep a `.env` file, and how is configuration shared between Docker and Spring
  ...
  application container; `application.yml` only *references* them via `${...}` placeholders
  ...
  toggle) live solely in `application.yml`.
- Q: How should Spring profiles be structured across dev and prod containers? → A: The base
  `application.yml` holds production-safe defaults and IS the production configuration — there
  is no separate `application-prod.yml` and the prod container activates no extra profile.
  `dev` is the only profile overlay (`application-dev.yml`), adding developer conveniences
  (e.g. `show-sql`, verbose logging) on top of the base; the dev container sets
  `SPRING_PROFILES_ACTIVE=dev`.
- Q: Which file FORMAT should the consolidated Spring configuration use? → A: YAML — the
  configuration stays in `application.yml` (plus `application-{profile}.yml`), the project's
  existing format; NO `application.properties` is introduced. This reverses an earlier `.properties`
  decision: only the file format changes. Every other mandate holds — no `.env`, no dotenv, secrets
  and non-secret connection values injected as environment variables by Docker Compose (single
  source of truth), and the base-as-prod / `dev`-overlay profile structure above. Aligns with
  constitution v1.1.2.
...
`application.yml` (plus profile-specific variants), and secrets appear there only as
...
passes with **no** secrets set; confirm all configuration is in `application.yml`, no
`application.properties` is introduced, and no `.env`/dotenv is read.
...
3. **Given** the repository, **When** its configuration files are inspected, **Then** all Spring
   configuration is in `application.yml`/`application-{profile}.yml`, no
   `application.properties` exists, and secrets appear only as `${...}` placeholders.
...
- **FR-001**: All Spring application configuration MUST reside in `application.yml` and any
  profile-specific `application-{profile}.yml`; the project's existing YAML configuration files
  (main, `dev`, and test) are RETAINED as the configuration surface and no `application.properties`
  may be introduced. The base `application.yml` MUST hold production-safe defaults and serve as the
  production configuration directly (no `application-prod.yml`, and the prod container activates no
  extra profile). `dev` MUST be the only profile overlay (`application-dev.yml`), adding developer
  conveniences on top of the base, with the dev container setting `SPRING_PROFILES_ACTIVE=dev`.
...
- **SC-005**: 100% of the application's Spring configuration resides in `application.yml`
  files; no `application.properties` and no `.env` is required for the application to run.
